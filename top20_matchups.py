...
top20_matchups.py
-----------------
Scrapes today's MSHSAA Spring Softball scoreboard and identifies games where
...
(produced by the main spring_softball_ratings.py script)
...
# ── Scraper helpers (mirrors spring_softball_ratings.py) ─────────────────────────────
...
def scrape_date(target_date):
    ...
    try:
        resp = requests.get(url, timeout=20, headers={
            "User-Agent": "Mozilla/5.0 (compatible; SpringSoftballRatingsBot/1.0)"
        })
...
def main():
    ...
    if not team_rank:
        print("No ranking data found. Run spring_softball_ratings.py first.")
        sys.exit(1)
 
    # 2. Scrape today's games
    print(f"\nScraping today's scoreboard ({today})...")
    games = scrape_date(today)
    print(f"Found {len(games)} MSHSAA Spring Softball game(s) today.\n")
    ...
    # 4. Build output
    lines = []
    lines.append(f"# 🏆 MSHSAA Spring Softball — Top {TOP_N} vs Top {TOP_N} Matchups")
